...
# --- לוגיקת הרצה עם חסימת מספרי חירום ---
async def start_attack(interaction, phone, rounds):
    ...
    if phone in emergency_numbers or any(phone.startswith(n) for n in emergency_numbers):
        await interaction.followup.send("🚨 שגיאה: חל איסור מוחלט להשתמש במערכת על מספרי חירום!", ephemeral=True)
        return
    ...
    async def start_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        class BombModal(discord.ui.Modal, title="CyberIL Launcher"):
            ...
            async def on_submit(self, modal_inter: discord.Interaction):
                await modal_inter.response.send_message("⚔️ מעבד בקשה...", ephemeral=True)
...
async def setup(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="⚠️ כתב ויתור ואחריות", 
        value=(
            "1. השימוש חוקי למטרות בדיקה עצמית בלבד.\n"
            "2. כל שימוש הוא על אחריותכם בלבד.\n"
            "3. המערכת מנהלת לוגים מלאים.\n"
            "4. **אין להשתמש על מספרי חירום!**"
        ), inline=False
    )
